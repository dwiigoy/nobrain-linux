...
static int xcallback(int fd, int events, __unused void* data) {
    ...
    log(ERROR, "xcallback: fd=%d events=0x%x conn_fd=%d", fd, events, conn_fd);

    // FIX (Build33): drain any pending data BEFORE tearing down on HANGUP/ERROR.
    // On the very first connect_(client[0]), closing the old conn_fd=client[1]
    // makes client[0] HUP immediately even though buffer-0's EVENT_ADD_BUFFER is
    // already sitting in its recv queue. Previously the HANGUP branch returned
    // early and that data (and rendererAddBuffer call) was silently discarded,
    // leaving the renderer with zero buffers -> permanent black screen.
    if (conn_fd != -1) {
        lorieEvent e = {0};
    ...
    if (events & (ALOOPER_EVENT_ERROR | ALOOPER_EVENT_HANGUP)) {
        log(ERROR, "xcallback: HANGUP/ERROR on fd=%d (events=0x%x), tearing down conn_fd=%d", fd, events, conn_fd);
...
static void connect_(__unused JNIEnv* env, __unused jobject cls, jint fd) {
    log(ERROR, "connect_: called with fd=%d, current conn_fd=%d", fd, conn_fd);
    if (conn_fd != -1) {
        ALooper_removeFd(ALooper_forThread(), conn_fd);
        // EXPERIMENT (Build34): do NOT close(conn_fd) here. conn_fd at this point
        // is client[1] (the X-server's own end of the socketpair, set by addFd()),
        // which is the PEER of the new fd=client[0] we're about to register with
        // ALooper. Closing the peer immediately HUPs client[0] (xcallback fires
        // with events=0x9 INPUT|HANGUP on its very first invocation), which tore
        // down conn_fd right after buffer 0 was drained, before any further
        // buffers could ever be registered. Testing whether leaving client[1]
        // open lets client[0] stay alive (events==0x1, no HANGUP).
        rendererSetSharedState(NULL);
        rendererRemoveAllBuffers();
        log(DEBUG, "disconnected");
    }
    ...
    if ((conn_fd = fd) != -1) {
        // FIX: Remove HANGUP monitoring so xcallback only fires for actual data.
        // Original HANGUP monitoring caused state events to be discarded when
        // connect_(client[0]) closes old conn_fd=client[1] (socketpair HANGUP).
        ALooper_addFd(ALooper_forThread(), fd, 0, ALOOPER_EVENT_INPUT, xcallback, NULL);
        log(DEBUG, "XCB connection is successfull");
    }
    log(ERROR, "connect_: done, conn_fd is now %d", conn_fd);
}
...
static void sendClipboardAnnounce(__unused JNIEnv *env, __unused jobject thiz) {
    log(ERROR, "sendClipboardAnnounce called, conn_fd=%d", conn_fd);
    if (conn_fd != -1) {
        lorieEvent e = { .type = EVENT_CLIPBOARD_ANNOUNCE };
        write(conn_fd, &e, sizeof(e));
        log(ERROR, "sendClipboardAnnounce: wrote EVENT_CLIPBOARD_ANNOUNCE");
    }
}
