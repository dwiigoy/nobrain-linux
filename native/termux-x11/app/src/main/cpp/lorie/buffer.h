 ...
 * @param buffer
 * @param socketFd
 * @return 0 on success, or a negative errno if the AHardwareBuffer handle
 *         (for LORIEBUFFER_AHARDWAREBUFFER buffers) could not be sent. The
 *         peer is informed either way, so it will not block in recvmsg()
 *         waiting for a handle that was never sent.
 */
int LorieBuffer_sendHandleToUnixSocket(LorieBuffer* _Nonnull buffer, int socketFd);
