...
// FIX (Layer D): conn_fd gets overwritten to client[0] by activity.c's connect_()
// right after addFd() sets it to client[1]. From that point on, conn_fd is the
// correct fd for "renderer -> server" writes (sendWindowChange etc, read by
// handleLorieEvents which is registered on client[1] but fed by writes to
// client[0]) but WRONG for "server -> renderer" writes: lorieRegisterBuffer()
// et al wrote EVENT_ADD_BUFFER to conn_fd=client[0], which lands in client[1]'s
// queue (consumed by handleLorieEvents, which has no EVENT_ADD_BUFFER case) and
// is never seen by activity.c's xcallback (registered on client[0]). The only
// reason buffer 0 ever got through was that lorieActivityConnected() ran from
// addFd() while conn_fd still equaled client[1] (correct direction), before
// connect_() flipped it. Any later lorieRegisterBuffer() call (e.g. the new
// root buffer created by a RandR resize on rotation) used the flipped conn_fd
// and was silently lost -> renderer waits forever for a buffer it never
// receives -> permanent black screen after rotation.
// renderer_fd keeps client[1] (the fd that, when written, is delivered to
// xcallback on client[0]) for the whole connection lifetime, independent of
// what connect_() later does to conn_fd.
static volatile int renderer_fd = -1;
    ...
    if (read(fd, &e, sizeof(e)) == sizeof(e)) {
        switch(e.type) {
            case EVENT_SCREEN_SIZE: {
                ...
                if (!copy) {
                    log(ERROR, "Failed to allocate %zu bytes for EVENT_SCREEN_SIZE (name_size %zu), dropping event",
                        sizeof(lorieEvent) + e.screenSize.name_size + 1, e.screenSize.name_size);
                    break;
                }
            ...
            case EVENT_CLIPBOARD_ANNOUNCE:
                __android_log_print(ANDROID_LOG_ERROR, "LorieNative", "handleLorieEvents: got EVENT_CLIPBOARD_ANNOUNCE");
...
void lorieSendClipboardData(const char* data) {
    if (data && renderer_fd != -1) {
        size_t len = strlen(data);
        lorieEvent e = { .clipboardSend = { .t = EVENT_CLIPBOARD_SEND, .count = len } };
        write(renderer_fd, &e, sizeof(e));
        write(renderer_fd, data, len);
    }
}
...
static Bool addFd(__unused ClientPtr pClient, void *closure) {
    ...
    renderer_fd = (int) (int64_t) closure;
...
void lorieRegisterBuffer(LorieBuffer* buffer) {
    ...
    if (renderer_fd == -1 || LorieBufferList_findById(&registeredBuffers, id))
        return; // Already registered

    if (renderer_fd != -1 && buffer) {
        lorieEvent e = { .type = EVENT_ADD_BUFFER };
        const LorieBuffer_Desc* desc = LorieBuffer_description(buffer);
        write(renderer_fd, &e, sizeof(e));
        if (LorieBuffer_sendHandleToUnixSocket(buffer, renderer_fd) != 0)
            log(ERROR, "Failed to send handle for buffer id %llu, renderer will not be able to display it", desc->id);
