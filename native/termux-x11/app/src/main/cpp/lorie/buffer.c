...
__LIBC_HIDDEN__ int LorieBuffer_lock(LorieBuffer* buffer, void** out) {
    ...
    if (buffer->desc.type == LORIEBUFFER_REGULAR || buffer->desc.type == LORIEBUFFER_FD)
        ...
        if (!buffer->desc.buffer) {
            loge("LorieBuffer_lock: buffer %llu has no backing AHardwareBuffer, returning ENODEV", (unsigned long long) buffer->desc.id);
            return ENODEV;
        }
...
__LIBC_HIDDEN__ int LorieBuffer_sendHandleToUnixSocket(LorieBuffer* _Nonnull buffer, int socketFd) {
    ...
    if (buffer->desc.type == LORIEBUFFER_FD)
        ...
        int ret = AHardwareBuffer_sendHandleToUnixSocket(buffer->desc.buffer, socketFd);
        uint8_t ok = ret == 0;
        // Let the peer know whether the handle below is actually coming, so
        // it does not block in recvmsg() forever if we failed to send it.
        write(socketFd, &ok, sizeof(ok));
        if (ret != 0) {
            dprintf(2, "AHardwareBuffer_sendHandleToUnixSocket failed for buffer %llu: %s\n", (unsigned long long) buffer->desc.id, strerror(-ret));
            loge("LorieBuffer_sendHandleToUnixSocket: AHardwareBuffer_sendHandleToUnixSocket failed for buffer %llu: %s", (unsigned long long) buffer->desc.id, strerror(-ret));
            return ret;
        }
    }
...
__LIBC_HIDDEN__ void LorieBuffer_recvHandleFromUnixSocket(int socketFd, LorieBuffer** outBuffer) {
    ...
    if (buffer.desc.type == LORIEBUFFER_FD) {
        ...
        uint8_t ok = 0;
        if (read(socketFd, &ok, sizeof(ok)) != sizeof(ok) || !ok) {
            dprintf(2, "AHardwareBuffer handle for buffer %llu was not sent by the peer, it will have no backing storage\n", (unsigned long long) buffer.desc.id);
            loge("LorieBuffer_recvHandleFromUnixSocket: AHardwareBuffer handle for buffer %llu was not sent by the peer, it will have no backing storage", (unsigned long long) buffer.desc.id);
            buffer.desc.buffer = NULL;
        } else {
            int ret = AHardwareBuffer_recvHandleFromUnixSocket(socketFd, &buffer.desc.buffer);
            if (ret != 0) {
                dprintf(2, "AHardwareBuffer_recvHandleFromUnixSocket failed for buffer %llu: %s\n", (unsigned long long) buffer.desc.id, strerror(-ret));
                loge("LorieBuffer_recvHandleFromUnixSocket: AHardwareBuffer_recvHandleFromUnixSocket failed for buffer %llu: %s", (unsigned long long) buffer.desc.id, strerror(-ret));
                buffer.desc.buffer = NULL;
            } else {
                loge("LorieBuffer_recvHandleFromUnixSocket: buffer %llu received OK, AHardwareBuffer=%p", (unsigned long long) buffer.desc.id, (void*) buffer.desc.buffer);
            }
        }
    }
