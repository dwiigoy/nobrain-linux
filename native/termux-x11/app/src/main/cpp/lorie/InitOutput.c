...
void OsVendorInit(void) {
    ...
    // Allow the Java side to override the compiled-in root window size before fbScreenInit()
    // creates buffer 0, since patching the .data defaults in libXlorie.so does not reliably
    // take effect (the patched copy can be deduped against an already-loaded same-soname mapping).
    {
        const char *w = getenv("RESOLUTION_X"), *h = getenv("RESOLUTION_Y");
        int iw = w ? atoi(w) : 0, ih = h ? atoi(h) : 0;
        if (iw > 0)
            lorieScreen.root.width = iw;
        if (ih > 0)
            lorieScreen.root.height = ih;
    }
...
void lorieActivityConnected(void) {
    ...
    lorieEnableClipboardSync(TRUE);
    /* Claim CLIPBOARD ownership immediately so paste-into-Linux always works.
     * This makes Android clipboard available without waiting for change listener. */
